...
Cucumber::Rake::Task.new(:acceptance) do |task|
  task.cucumber_opts = ['features', '--tags \'not @wip and not @api and not @bot and not @local\'', '--format pretty', '--format html -o reports/cucumber.html']
end
...
if %w[development test].include?(RACK_ENV)
  ...
  Cucumber::Rake::Task.new(:cucumber) do |task|
    ...
    task.cucumber_opts = ['features', '--tags \'not @wip and not @bot and not @api\'']
  end
